...
function verticalSlide(direction) {
    ...
    for (let i = 0; i < GRID_SIZE; i++) {
        column = [];
        //get column
        for (let j = i; j < GRID_SIZE * GRID_SIZE; j += 4) {
            column.push(grid[j]);
        }
...
function displayGameOver() {
    displayText("Game Over!\nHit Enter to Play Again", color(255, 0, 0), 32, width / 2, height / 2);
}

/* displays the Game Won message */
function displayGameWon() {
    displayText("You Win!\nHit Enter to Play Again", color(124,252,0), 32, width / 2, height / 2);
}
